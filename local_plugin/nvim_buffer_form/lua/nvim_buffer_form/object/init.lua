...
local idx = {subclasses = {"<nvim_buffer_form.object:subclasses>"}}

local function __tostring(self) return "class " .. self.name end

local function __call(self, ...) return self:new(...) end

local function create_index_wrapper(class, index)
    if type(index) == "table" then
        return function(self, key)
            local value = self.class.__meta[key]
            if value == nil then return index[key] end
            return value
        end
    elseif type(index) == "function" then
        return function(self, key)
            local value = self.class.__meta[key]
            if value == nil then return index(self, key) end
            return value
        end
    else
        return class.__meta
...
local function declare_instance_property(class, key, value)
    ...
    if value == nil and class.super then value = class.super.__meta[key] end

    propagate_instance_property(class, key, value)
end

local function is_subclass(subclass, class)
    if not subclass.super then return false end
    if subclass.super == class then return true end
    return is_subclass(subclass.super, class)
end

local function is_instance(instance, class)
    if instance.class == class then return true end
...
local function create_class(name, super)
    ...
    local meta = {is_instance_of = is_instance}
    meta.__index = meta

    local class = {
        super = super,
        name = name,
        static = {is_subclass_of = is_subclass},
    ...
    setmetatable(class.static, {
        __index = function(_, key)
            ...
            if value == nil and super then return super.static[key] end
            return value
        end
    })
...
local function create_object(_, name)
    ...
    ---@return string
    function Class:__tostring() return "instance of " .. tostring(self.class) end
    ...
    function Class.static:extend(name) -- luacheck: no redefined
        ...
        function subclass.init(instance, ...) self.init(instance, ...) end
